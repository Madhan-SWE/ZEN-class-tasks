let chunk = (arr, chunkSize = 1) => {
  let arrLength = arr === null ? 0 : arr.length;
  ...
  let res = new Array(resSize);

  let totalChunks = resSize;
  let currentChunk = 1;
  ...
  for (let i = 0; i < arrLength; i++) {
    ...
    if (currentChunkIdx === 0) {
      if (currentChunk * chunkSize < arrLength) {
        ...
        let chunkResSize = chunkSize - (currentChunk * chunkSize - arrLength);
...
let displayChunkres = (chunkRes) => {
  for (let i = 0; i < chunkRes.length; i++) {
...
let chunkRes = chunk([1, 2, 3, 4, 5, 6, 7, 8, 9], 8);
...
let reduce = (objInput, func, accumulator) => {
  let res = [];
  let idx = 0;
  if (Object.prototype.toString.call(objInput) === "[object Array]") {
    let prevRes = accumulator;
...
let filter = (objInput, iteratee) => {
  let res = [];
  let idx = 0;
  let objLength = objInput.length;
...
console.log("#########################################################");
console.log("==================Testcase 1:===================");
let filterRes = filter(users, function (o) {
  return o.age < 40;
});
...
console.log("#########################################################");
console.log("==================Testcase 1:===================");
let findRes = find(users, function (o) {
  return o.age < 40;
});
